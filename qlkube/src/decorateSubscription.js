...
function getResourceApiMainType(fieldName) {
  return subscriptions.filter(sub => {
    return `${sub.type}Update` === fieldName;
  })[0];
}
  ...
  fieldWrapper.forEach(fw => {
    if (!resultCheck) {
      ...
      if (isList) {
            ...
            resultCheck =
              ...
              ((item.metadata.namespace ||
                ...
                (item.metadata.name === undefined ||
                  item.metadata.name === payload.apiObj.metadata.name));
              ...
              resultCheck =
                ...
                ((targetObjField.namespace ||
                  ...
                  (targetObjField.name === undefined ||
                    targetObjField.name === payload.apiObj.metadata.name));
            }
            if (resultCheck) {
              graphqlLogger(
                `[i] Item found: ${JSON.stringify(
         ...
         */
        graphqlLogger(
          `[i] Checks if the published event is about the main type of the item: ${
            payload.apiObj
          } (subscription: ${fieldName} with variables: ${JSON.stringify(
            variables
          )})`
        ...
        resultCheck =
          ...
          ((payload.apiObj.metadata.namespace ||
            ...
            (newApiObj.metadata.name === undefined ||
              payload.apiObj.metadata.name === newApiObj.metadata.name));
        ...
        if (typeof targetObjField === 'object') {
          resultCheck =
            resultCheck ||
            ((targetObjField.namespace ||
              targetObjField.namespace === payload.apiObj.metadata.namespace) &&
              (targetObjField.name === undefined ||
                targetObjField.name === payload.apiObj.metadata.name));
        }
...
function decorateSubscription(baseSchema, targetType, enumType, kubeApiUrl) {
  ...
  type ${subscriptionType} {
    ...
    ${subType} {
      ${subscriptionField}(
        name: ${
          resourceApiMainType.resource === 'tenants' ? 'String!' : 'String'
        }
        namespace: ${
          resourceApiMainType.resource === 'tenants' ? 'String' : 'String!'
        }): ${subscriptionType}
